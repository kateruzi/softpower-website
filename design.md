...
`connection` с 25.09.2026 в `old_website/connection-2026.html`: подписки больше нет, практики — в бесплатном приложении.
Классы, описанные только на ней (`.plan`, центрированный hero), остаются справкой.
...
| `praktika` | hero → statement → медиа-блок «бесплатное приложение soft power» (`#app`, фото + текст + кнопка в приложение) → форматы `.fcard` ×2 → ретрит `.fcard` → soft power pass → футер |
...
| `pass` | hero → варианты `.card` ×3 → statement → закрывающая галерея 3/4 → футер |
| `connection` (с 25.09.2026 в `old_website/`, адрес ведёт на `/praktika#app`) | центрированный hero → «что внутри» (2 колонки: список + фото) → тарифы `.plan` ×3 → отзывы-карусель → футер |
